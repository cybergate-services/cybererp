...
class Channel(models.Model):
    ...
    def new_channel(self, values):
        if type(values) == list:
            values = values[0]
        #logger.info('NEW CHAN {}'.format(json.dumps(values, indent=2)))
        ...
        self.env['bus.bus'].sendone('asterisk_base_channels', 'reload')
        return True
    ...
    def update_channel_state(self, values):
        if type(values) == list:
            values = values[0]
        #logger.info('STATE UP {}'.format(json.dumps(values, indent=2)))
        ...
        if channel:
            ...
            logger.warning('No channel {} found for state update.'.format(
            values.get('Uniqueid')))
        self.env['bus.bus'].sendone('asterisk_base_channels', 'reload')
        return True
    ...
    def hangup_channel(self, values):
        if type(values) == list:
            values = values[0]
        #logger.info('HANGUP {}'.format(json.dumps(values, indent=2)))
        uniqueid = values.get('Uniqueid')
        channel = values.get('Channel')
